...
export function AdminPagination({ currentPage, totalPages, pageNumbers, onPageChange }: AdminPaginationProps) {
  const t = useTranslations("AdminDataView");
    ...
    <div className="flex items-center justify-between px-6 py-4 border-t border-border bg-muted/5">
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          aria-label={t("label_previous_page")}
        >
      ...
      <div className="flex items-center gap-3 mb-4">
        {Icon && (
          <div className={cn(
            ...
            destructive ? "bg-destructive/10 border-destructive/20" : "bg-primary/10 border-primary/20"
          )}>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t("this_action_cannot_be_undone")}</p>
        </div>
      </div>
...
export function InitialsAvatar({
  ...
  if (imageUrl && !imageUrl.includes("placeholders")) {
    return (
      <div className={cn(
        "w-9 h-9 rounded-full overflow-hidden shrink-0 bg-muted",
        className
      )}>
  ...
  return (
    <div className={cn(
      "w-9 h-9 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-semibold shrink-0",
      className
    )}>
      {getInitials(name)}
    </div>
