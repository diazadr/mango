...
export const SidebarDropdown = ({ item, pathname, isOpen, onToggle, collapsed }: Props) => {
  ...
  const isParentActive = pathname === item.href || (item.subItems?.some((sub: any) => {
    if (pathname === sub.href) return true;
    if (sub.exact) return false;
    if (pathname.startsWith(sub.href + "/") && (!sub.excludePaths || !sub.excludePaths.some((p: string) => pathname.startsWith(p)))) return true;
    return false;
  }));
    ...
    <div className="space-y-0.5">
      ...
      <div className={`ml-7 pl-4 border-l border-border transition-all duration-200 space-y-0.5 overflow-hidden ${
        ...
        {item.subItems?.map((sub: any) => {
          const isSubActive = pathname === sub.href || (!sub.exact && pathname.startsWith(sub.href + "/") && (!sub.excludePaths || !sub.excludePaths.some((p: string) => pathname.startsWith(p))));
          return (
