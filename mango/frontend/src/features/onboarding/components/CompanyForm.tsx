    ...
    <form onSubmit={onSubmit} className="space-y-8 pb-10">
      ...
      {Object.keys(errors).length > 0 && (
        <Alert variant="destructive" className="bg-destructive/5 border-destructive/20">
          ...
          <AlertTitle className="text-sm font-bold">{t("data_belum_lengkap")}</AlertTitle>
      ...
      <div className="space-y-6">
        <h3 className="text-lg font-bold text-foreground">{t("identitas_usaha")}</h3>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="flex flex-col items-center gap-2">
                ...
                >
                    {logoPreview ? (
                        <img src={logoPreview} alt={t("alt_logo")} className="h-full w-full object-cover" />
                    ) : (
                ...
                <p className="text-[10px] font-medium text-muted-foreground mt-2">{t("click_to_upload_logo")}</p>
            </div>

            <div className="flex-1 w-full space-y-4">
                <div className="space-y-2">
                    <Label className="text-sm font-medium text-muted-foreground ml-1">{t("company_name")}</Label>
                    <Input {...register("name")} placeholder={t("company_name_placeholder")} className="h-11 rounded-xl bg-background" />
                    {errors.name && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.name.message}`)}</p>}
                </div>
                <div className="space-y-2">
                    <Label className="text-sm font-medium text-muted-foreground ml-1">{t("legal_entity")}</Label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-dashed">
        <div className="space-y-2">
            <Label className="text-sm font-medium text-muted-foreground ml-1">{t("phone")}</Label>
            <Input {...register("phone")} placeholder={t("phone_placeholder")} className="h-11 rounded-xl bg-background" />
            {errors.phone && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.phone.message}`)}</p>}
        </div>

        <div className="space-y-2">
            <Label className="text-sm font-medium text-muted-foreground ml-1">{t("npwp")} (Opsional)</Label>
            <Input {...register("npwp")} placeholder={t("npwp_placeholder")} className="h-11 rounded-xl font-mono text-xs bg-background" />
        </div>
      </div>

      {/* 2. Legal Documentation Section */}
      <div className="space-y-6 pt-6 border-t">
        <h3 className="text-lg font-bold text-foreground">{t("dokumen_legalitas")}</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("unggah_dokumen_nib")}</Label>
                <div 
                ...
                >
                    {nibFileName ? (
                        ...
                        <>
                            ...
                            <span className="text-[10px] font-medium text-muted-foreground mt-2">{t("click_to_upload_file")}</span>
                        </>
                    )}
                </div>
      ...
      <div className="space-y-6 pt-6 border-t">
        <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-foreground">{t("lokasi_operasional")}</h3>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-2 md:col-span-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("address")}</Label>
                <Input {...register("address")} placeholder={t("address_placeholder")} className="h-11 rounded-xl bg-background" />
                {errors.address && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.address.message}`)}</p>}
            </div>

            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("province")}</Label>
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("regency")}</Label>
                <div className="relative">
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("kecamatan")}</Label>
                <div className="relative">
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("kelurahan_desa")}</Label>
                <div className="relative">
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("kode_pos")}</Label>
                <Input {...register("postal_code")} placeholder={t("placeholder_contoh_40123")} className="h-11 rounded-xl font-mono bg-background" maxLength={5} />
                {errors.postal_code && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.postal_code.message}`)}</p>}
            </div>

            <div className="space-y-4 md:col-span-2">
                <div className="space-y-2">
                    <Label className="text-sm font-medium text-muted-foreground ml-1">{t("sector")}</Label>
                    <select 
                    ...
                    >
                        ...
                        <option value="Industri Makanan & Minuman">{t("industri_makanan_minuman")}</option>
                        <option value="Industri Tekstil & Konveksi">{t("industri_tekstil_konveksi")}</option>
                        <option value="Industri Produk Kulit & Alas Kaki">{t("industri_produk_kulit_alas_kaki")}</option>
                        <option value="Industri Pengolahan Kayu & Furniture">{t("industri_pengolahan_kayu_furniture")}</option>
                        <option value="Industri Logam, Mesin & Elektronik">{t("industri_logam_mesin_elektronik")}</option>
                        <option value="Industri Kimia, Farmasi & Kosmetik">{t("industri_kimia_farmasi_kosmetik")}</option>
                        <option value="Industri Kerajinan & Barang Seni">{t("industri_kerajinan_barang_seni")}</option>
                        <option value="Industri Teknologi Informasi (Software/IT)">{t("industri_teknologi_informasi_softwareit")}</option>
                        <option value="Lainnya">{t("lainnya_sebutkan")}</option>
                    </select>
                </div>
                ...
                {(watch("sector") === "" || !["Industri Makanan & Minuman", "Industri Tekstil & Konveksi", "Industri Produk Kulit & Alas Kaki", "Industri Pengolahan Kayu & Furniture", "Industri Logam, Mesin & Elektronik", "Industri Kimia, Farmasi & Kosmetik", "Industri Kerajinan & Barang Seni", "Industri Teknologi Informasi (Software/IT)"].includes(watch("sector"))) && (
                    ...
                    >
                        <Label className="text-sm font-medium text-primary ml-1">{t("sektor_spesifik")}</Label>
                        <Input 
                            {...register("sector")} 
                            placeholder={t("placeholder_sebutkan_sektor_industri_anda_secara_spe")} 
                            className="h-11 rounded-xl bg-primary/5 border-primary/20 focus:border-primary"
                        />
                    </motion.div>
                )}
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("nomor_induk_berusaha_nib")}</Label>
                <Input {...register("nib")} maxLength={13} className="h-11 rounded-xl font-mono text-sm bg-background" placeholder={t("placeholder_masukkan_nib")} />
                {errors.nib && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.nib.message}`)}</p>}
            </div>

            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("tahun_berdiri")}</Label>
                <select 
                    {...register("established_year", { valueAsNumber: true })}
                    className="flex h-11 w-full rounded-xl border border-input bg-background px-4 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-0 focus-visible:border-primary disabled:opacity-50 transition-all"
            ...
            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("jumlah_karyawan")}</Label>
                <Input {...register("employee_count", { valueAsNumber: true })} type="number" min={0} placeholder={t("placeholder_contoh_5")} className="h-11 rounded-xl bg-background" />
                {errors.employee_count && <p className="text-[10px] font-bold text-destructive ml-1">{t(`errors.${errors.employee_count.message}`)}</p>}
            </div>

            <div className="space-y-2">
                <Label className="text-sm font-medium text-muted-foreground ml-1">{t("afiliasi_organisasi_jika_ada")}</Label>
                <select {...register("organization_id", { valueAsNumber: true })} className={`flex h-11 w-full rounded-xl border bg-background px-3 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-0 focus-visible:border-primary ${errors.organization_id ? 'border-destructive' : 'border-input'}`}>
                    <option value="0">-- Tidak Ada / Mandiri --</option>
                    {organizations.map((org) => (
                        <option key={org.id} value={org.id}>{org.name}</option>
                    ))}
                </select>
                <p className="text-[10px] text-muted-foreground mt-1 ml-1 italic">{t("pilih_organisasi_atau_paguyuban_jika_and")}</p>
            </div>
        </div>
      </div>
