...
export const ConfirmPasswordModal = ({ 
    ...
    const t = useTranslations("ConfirmPasswordModal");
      ...
      <DialogContent className="sm:max-w-[425px] rounded-[2rem]">
        ...
        <form onSubmit={handleConfirm} className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="confirm-password-input" className="text-xs font-bold text-muted-foreground ml-1">{t("password")}</Label>
            <Input 
                ...
                placeholder={t("masukkan_kata_sandi_anda")}
            />
            {error && <p className="text-[10px] font-medium text-destructive ml-1">{error}</p>}
          </div>
          <DialogFooter className="gap-2 sm:gap-0 mt-6">
            <Button type="button" variant="ghost" onClick={onClose} className="rounded-xl h-11 font-bold flex-1">{t("batal")}</Button>
            <Button type="submit" disabled={isSubmitting || !password} className="rounded-xl h-11 font-bold bg-primary text-white flex-1">
              {isSubmitting && <Loader2 className="animate-spin" size={18} />} {t("konfirmasi")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
