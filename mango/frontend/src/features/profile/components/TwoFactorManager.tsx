...
export const TwoFactorManager = ({ 
  ...
  onEnable, onConfirm, onDisable, onShowRecovery, onRegenerateRecovery
    ...
    const t = useTranslations("TwoFactorManager");
      ...
      <div className="space-y-6 py-2 animate-in fade-in duration-500">
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-1">
            <p className="text-sm font-bold">{t("siapkan_autentikasi_dua_faktor")}</p>
            <p className="text-xs text-muted-foreground max-w-xs">{t("pindai_kode_qr_di_atas_menggun")}</p>
          </div>
        </div>

        <div className="space-y-4 bg-muted/20 p-5 rounded-2xl border">
          <div className="space-y-2">
            <Label className="text-xs font-bold text-muted-foreground ml-1">{t("kode_konfirmasi")}</Label>
            <div className="flex gap-2">
                <Input 
                    value={confirmCode}
                    onChange={(e) => setConfirmCode(e.target.value)}
                    placeholder={t("contoh_123456")}
                ...
                >
                    {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : null} {t("verifikasi")}
                </Button>
            </div>
          </div>
        </div>
        ...
        {recoveryCodes.length > 0 && (
            <div className="space-y-3">
                <p className="text-[10px] font-bold text-muted-foreground tracking-wide">{t("simpan_kode_pemulihan_ini")}</p>
  ...
  return (
    <div className="space-y-6 py-2">
      <div className="flex items-center justify-between py-2">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
                <p className="text-sm font-bold text-foreground">{t("status_2fa")}</p>
        ...
        >
          {isSubmitting ? <Loader2 className="animate-spin" size={18} /> : null} {isEnabled ? "Nonaktifkan" : "Aktifkan"}
        </Button>
      </div>
      ...
      {isEnabled && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
                <Key size={16} strokeWidth={1.5} /> {t("lihat_kode_pemulihan")}
            </Button>
            <Button 
                variant="outline" 
                size="sm" 
                ...
                className="h-12 rounded-2xl border-border/50 font-bold text-xs flex gap-2"
            >
                {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} strokeWidth={1.5} />} {t("buat_ulang_kode")}
            </Button>
        </div>
      )}
      ...
      {recoveryCodes.length > 0 && !isConfirming && (
          <div className="p-6 rounded-[2rem] bg-warning/5 border border-warning/10 space-y-4 animate-in slide-in-from-top-4 duration-500">
              <div className="flex items-center gap-3 text-warning">
                  ...
                  <p className="text-sm font-black">{t("kode_pemulihan_cadangan")}</p>
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">{t("simpan_kode_kode_ini_di_tempat")}</p>
