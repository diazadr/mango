...
import { 
  ...
  Settings2, CheckCircle2, Save, MapPin, Zap, CalendarClock,
} from "lucide-react";
...
// ── Komponen ──────────────────────────────────────────────────────────────────
export function AdminCampusTechnicalProfileView() {
  const t = useTranslations("AdminCampusTechnicalProfileView");
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const campusData = user.organizations?.[0] || user.institutions?.[0];
      if (campusData) {
      ...
      console.error("Gagal mengambil data teknis kampus:", err);
    } finally {
      setLoading(false);
    }
  ...
  const filteredMachines = useMemo(() => {
    return machines
      .filter((m) => {
        ...
        const matchReservable  = reservableFilter  === "all" ||
  ...
  const handleMachineSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!editingMachine) {
        const ownerType = (user?.organizations?.length ?? 0) > 0 ? "organization" : "institution";
        formData.append("owner_type", ownerType);
        formData.append("owner_id", String(campus.id));
      }
      ...
      setStatus({ type: "destructive", message: msg });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <DashboardPageShell
      title={t("title_profil_permesinan_institusi")}
      subtitle={t("title_pusat_manajemen_aset_permesinan_institus")}
      icon={Settings2}
    >
                ...
                <AdminTable>
                  <AdminTableHeader>
                    <AdminTableRow>
                      ...
                      <SortableHeader label={t("label_merktipe")} sortKey="brand"       currentSort={sortKey} direction={sortOrder} onSort={handleSort} />
                    ...
                    {filteredMachines.length === 0 ? (
                      ...
                      return (
                        <AdminTableRow key={item.id}>
                          ...
                          <AdminTableCell>
                            {item.is_reservable
                              ? <span className="text-sm font-semibold text-foreground">{formatRp(item.hourly_rate || 0)}</span>
                              : <span className="text-sm text-muted-foreground">—</span>}
                          </AdminTableCell>
                          {/* Ketersediaan */}
                          <AdminTableCell>
                          ...
                          <AdminTableCell align="right">
                            <div className="flex justify-end gap-1">
                              <AdminIconButton onClick={() => openMachineDialog(item)} title={t("title_edit")} tone="primary">
                                <Pencil size={14} />
                              </AdminIconButton>
                              <AdminIconButton onClick={() => deleteMachine(item.id)} title={t("title_hapus")} tone="destructive">
                                <Trash2 size={14} />
                              </AdminIconButton>
                            </div>
                          </AdminTableCell>
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={machineForm.name} onChange={(e) => setMachineForm({ ...machineForm, name: e.target.value })} placeholder={t("placeholder_cnc_makino")} className="h-11 rounded-xl bg-background border-input" required />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-muted-foreground ml-1">{t("kode_id")}</Label>
                  <Input value={machineForm.code} onChange={(e) => setMachineForm({ ...machineForm, code: e.target.value })} placeholder={t("placeholder_mch01")} className="h-11 rounded-xl bg-background border-input font-mono text-xs" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={machineForm.brand} onChange={(e) => setMachineForm({ ...machineForm, brand: e.target.value })} placeholder={t("placeholder_dmg_mori")} className="h-11 rounded-xl bg-background border-input" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-muted-foreground ml-1">{t("tipe_model")}</Label>
                  <Input value={machineForm.type} onChange={(e) => setMachineForm({ ...machineForm, type: e.target.value })} placeholder={t("placeholder_horizontal_milling")} className="h-11 rounded-xl bg-background border-input" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input type="number" min={1} value={machineForm.quantity} onChange={(e) => setMachineForm({ ...machineForm, quantity: e.target.value })} className="h-11 rounded-xl bg-background border-input" />
                </div>
                <div className="space-y-2">
                  ...
                  <Select value={machineForm.condition} onValueChange={(v) => setMachineForm({ ...machineForm, condition: v })}>
                    <SelectTrigger className="h-11 rounded-xl bg-background border-input"><SelectValue /></SelectTrigger>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={machineForm.location} onChange={(e) => setMachineForm({ ...machineForm, location: e.target.value })} placeholder={t("placeholder_workshop_a_lantai_1")} className="h-11 rounded-xl bg-background border-input" />
                </div>
                <div className="space-y-2">
                  <Label className="text-xs font-bold text-muted-foreground ml-1">{t("catatan_internal")}</Label>
                  <Input value={machineForm.notes} onChange={(e) => setMachineForm({ ...machineForm, notes: e.target.value })} placeholder={t("placeholder_perlu_kalibrasi_rutin")} className="h-11 rounded-xl bg-background border-input" />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-muted/20 border border-border/50">
                ...
                <Checkbox checked={machineForm.is_iot_enabled} onCheckedChange={(checked) => setMachineForm({ ...machineForm, is_iot_enabled: checked === true })} className="w-5 h-5 data-[state=checked]:bg-primary data-[state=checked]:border-primary cursor-pointer" />
              </div>
              ...
              <div className="pt-4 border-t border-dashed border-border/50 space-y-4">
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-muted/20 border border-border/50">
                  ...
                  <Checkbox checked={machineForm.is_reservable} onCheckedChange={(checked) => setMachineForm({ ...machineForm, is_reservable: checked === true })} className="w-5 h-5 data-[state=checked]:bg-primary data-[state=checked]:border-primary cursor-pointer" />
                </div>
                ...
                <div className={`space-y-2 transition-all duration-300 ${!machineForm.is_reservable ? "opacity-40 pointer-events-none" : ""}`}>
                  ...
                  <Textarea value={machineForm.description} onChange={(e) => setMachineForm({ ...machineForm, description: e.target.value })} className="rounded-xl bg-background border-input h-24 resize-none text-sm" placeholder={t("placeholder_jelaskan_spesifikasi_atau_ketentuan_rese")} />
                </div>
              </div>
            </div>

            <DialogFooter className="p-6 pt-2 border-t border-border/50">
              <Button type="button" variant="ghost" onClick={() => setMachineDialogOpen(false)} className="rounded-xl">
                Batal
              </Button>
              <Button type="submit" disabled={submitting} className="flex-1 h-11 rounded-xl font-bold bg-primary shadow-lg shadow-primary/20">
                {submitting
