      ...
      <CardHeader className="bg-muted/30 pb-4 border-b border-border/30">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-base font-bold text-primary tracking-tight">{member.name}</CardTitle>
              <CardDescription className="text-[10px] font-medium flex items-center gap-1">
                <Mail size={10} /> {member.email}
              </CardDescription>
            </div>
          </div>
          <Badge className="bg-warning/10 text-warning border-warning/20 text-[9px] font-black">{t("pending")}</Badge>
        </div>
      </CardHeader>
      <CardContent className="p-6 space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <p className="text-[10px] font-black text-muted-foreground tracking-wide">{t("mendaftar_pada")}</p>
            <p className="text-xs font-bold text-foreground">
              {format(new Date(member.created_at), "dd MMM yyyy", { locale: localeId })}
            </p>
          </div>
          <div className="space-y-1 text-right">
            <p className="text-[10px] font-black text-muted-foreground tracking-wide">{t("peran_sistem")}</p>
