        ...
        <div className="p-5 rounded-xl bg-primary/5 border border-primary/10 space-y-4 animate-in slide-in-from-top-2 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
                ...
                <Input 
                    ...
                    className="h-10 text-sm rounded-xl bg-background border-input focus:border-primary transition-all" 
                    value={newCert.name}
            ...
            <div className="space-y-1.5">
                ...
                <DateTimePicker 
                    value={newCert.issued_date ? new Date(newCert.issued_date) : null}
                    onChange={d => setNewCert({...newCert, issued_date: d ? format(d, 'yyyy-MM-dd') : ""})}
                />
            </div>
            <div className="space-y-1.5">
                <Label className="text-xs font-bold text-muted-foreground ml-1">{t("cert_expiry_label")}</Label>
                <DateTimePicker 
                    value={newCert.expiry_date ? new Date(newCert.expiry_date) : null}
                    onChange={d => setNewCert({...newCert, expiry_date: d ? format(d, 'yyyy-MM-dd') : ""})}
                />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input 
                type="file" 
                accept=".pdf,image/*" 
                className="h-10 text-xs bg-background border-input rounded-xl file:bg-primary/10 file:text-primary file:font-bold file:rounded-lg file:border-0 file:px-3 file:mr-3"
      ...
      <div className="grid grid-cols-1 gap-3">
        {umkm.certification_docs?.length > 0 ? (
          umkm.certification_docs.map((doc: any) => (
            <div key={doc.id} className="flex items-center justify-between p-4 rounded-xl bg-card border border-primary/20 shadow-sm group hover:border-primary/40 transition-colors">
          ...
          <div className="text-center py-10 rounded-xl border border-dashed border-border bg-muted/5">
