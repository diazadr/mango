...
class UmkmController extends Controller
{
    ...
    /**
     * Perkembangan UMKM: Timeline asesmen dan pendampingan.
     */
    public function progressHistory(Umkm $umkm): JsonResponse
    {
        $this->authorize('view', $umkm);
        ...
        try {
            ...
            // 1. Ambil semua asesmen yang sudah disubmit
            $assessments = \App\Models\Assessment\AssessmentResult::where('umkm_id', $umkm->id)
            ...
            foreach ($assessments as $assessment) {
                $history[] = [
                    ...
                    'is_mentoring' => $assessment->is_mentoring_result,
                    'scores'       => $assessmentService->calculateCategoryScores($assessment),
                ];
            }

            // 2. Ambil semua mentoring yang sudah selesai
            $mentoringRequests = \App\Models\Mentoring\ConsultationRequest::where('umkm_id', $umkm->id)
                ->where('status', 'done')
                ->with(['sessions.notes'])
                ->orderBy('updated_at', 'asc')
                ->get();

            foreach ($mentoringRequests as $request) {
                $history[] = [
                    'type'  => 'mentoring',
            ...
            Log::error('UMKM progress history error', ['message' => $e->getMessage()]);

            return $this->error('Gagal mengambil riwayat perkembangan.', 500);
        }
    }
}
