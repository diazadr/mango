...
class ArticleController extends Controller
{
    ...
    /**
     * @OA\Post(
     *     path="/admin/articles/upload-image",
     *     summary="Upload an image for article body",
     *     tags={"Admin Articles"},
     *     security={{"cookieAuth": {}}},
     *
     *     @OA\RequestBody(
    ...
    public function uploadImage(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $path = $image->store('articles/body', 'public');

            return response()->json([
                'url' => asset('storage/' . $path)
            ]);
        } catch (Throwable $e) {
            Log::error('Article image upload error', ['message' => $e->getMessage()]);
            return $this->error('Gagal mengunggah gambar.', 500);
        }
    }
}
