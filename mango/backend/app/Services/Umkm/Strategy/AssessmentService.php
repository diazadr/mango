...
class AssessmentService
{
    ...
    public function process(
        ...
        return DB::transaction(function () use ($result) {
            ...
            $totalScore = $this->calculateTotalScore($categoryScores);

            $level = $this->determineLevel($totalScore);

            // [FIX IDEMPOTENCY] Jangan overwrite submitted_at jika assessment sudah pernah di-submit
            $updates = [
            ...
            $this->generateRecommendations($result, $categoryScores);

            return $result->fresh(['recommendations']);
        });
    }
    ...
    public function calculateCategoryScores(
        ...
        $averages = Answer::query()
            ->join('questions', 'answers.question_id', '=', 'questions.id')
            ->where('answers.assessment_result_id', $result->id)
            ->selectRaw('questions.assessment_category_id, AVG(answers.score) as avg_score')
            ->groupBy('questions.assessment_category_id')
            ->pluck('avg_score', 'assessment_category_id');

        foreach ($categories as $category) {
            $avgScore = $averages[$category->id] ?? 0;
    ...
    protected function generateRecommendations(
        ...
        foreach ($categoryScores as $slug => $data) {
            ...
            $pillarLevel = $this->determineLevel($avg);
    ...
    /**
     * Menerapkan dampak pendampingan: menaikkan poin jawaban pada kategori yang diperbaiki
     * secara otomatis dengan membuat duplikat assessment agar riwayat awal tidak terhapus.
     */
    public function applyMentoringImpact(\App\Models\Umkm\Umkm $umkm, array $categoryIds): void
    {
        $latestAssessment = AssessmentResult::query()
        ...
        DB::transaction(function () use ($latestAssessment, $categoryIds) {
            // Replikasi hasil assessment menjadi riwayat baru
            $newAssessment = $latestAssessment->replicate();
            $newAssessment->submitted_at = null;  // Reset agar process() bisa set submitted_at baru
            // [TAMBAH] Tandai sebagai hasil dari proses mentoring
            $newAssessment->is_mentoring_result = true;
            $newAssessment->save();

            // Replikasi jawaban dan sesuaikan skor kategori yang di-mentoring
            $oldAnswers = Answer::with('question')
                ->where('assessment_result_id', $latestAssessment->id)
                ->get();

            $updated = false;

            foreach ($oldAnswers as $oldAnswer) {
                $newAnswer = $oldAnswer->replicate();
                $newAnswer->assessment_result_id = $newAssessment->id;

                if (in_array($oldAnswer->question->assessment_category_id, $categoryIds) && $newAnswer->score < 5) {
                    $newAnswer->score = min(5, $newAnswer->score + 1);
                    $updated = true;
                }
            ...
            if ($updated) {
                $this->process($newAssessment);
            } else {
                // Batalkan pembuatan hasil baru karena tidak ada poin yang bisa dinaikkan
                Answer::where('assessment_result_id', $newAssessment->id)->delete();
